import { Beam, SupportConfig, SupportType } from '@/types/beam.ts';
...
function drawSupport(support: SupportConfig, { ctx, beam, pixelsPerMeter, beamY, offsetX }: RenderContext) {
    // 处理负值位置：负值表示离远端的距离
    const pos = support.position < 0 ? beam.length + support.position : support.position;
...
function drawFixed(ctx: CanvasRenderingContext2D, x: number, y: number, isStart: boolean) {
    const height = 30;
    const width = 8;
