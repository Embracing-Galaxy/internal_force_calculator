import { Beam, LoadType } from '@/types/beam.ts';
...
export function BeamRenderer({ ctx, beam, pixelsPerMeter, beamY, offsetX }: RenderContext) {
...
function drawPointLoad(ctx: CanvasRenderingContext2D, x: number, beamY: number, magnitude: number) {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`${Math.abs(magnitude).toFixed(1)}kN`, x + 5, endY + (isUp ? -5 : 15));
}

function drawMomentLoad(ctx: CanvasRenderingContext2D, x: number, beamY: number, magnitude: number) {
    const radius = 20;
...
function drawDistributedLoad(ctx: CanvasRenderingContext2D, startX: number, endX: number, beamY: number, magnitude: number) {
    const isUp = magnitude > 0;
    ...
    ctx.font = 'bold 11px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(`${Math.abs(magnitude).toFixed(1)}kN/m`, (startX + endX) / 2, arrowY + (isUp ? -10 : 20));
}