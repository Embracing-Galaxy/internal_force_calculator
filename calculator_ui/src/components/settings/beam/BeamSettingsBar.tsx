...
import { Sidebar, SidebarContent, SidebarFooter } from "@/components/ui/sidebar.tsx";
import { Beam, Load, SupportType } from '@/types/beam.ts';
...
export default function BeamSettingsBar({ beam, onBeamChange, onCalculate }: SettingsBarProps) {
    const handleLengthChange = (length: number) => {
        // 检查并更新固定端位置
        const updatedSupportA = beam.supportA.type === SupportType.Fixed && beam.supportA.position !== 0
            ? { ...beam.supportA, position: length }
            : beam.supportA;

        const updatedSupportB = beam.supportB.type === SupportType.Fixed && beam.supportB.position !== 0
    ...
    return (
        <Sidebar side="right" variant="floating">
            ...
            <SidebarFooter>
                <Button className="w-full" onClick={onCalculate}>计算内力</Button>
            </SidebarFooter>
        </Sidebar>
    );
}