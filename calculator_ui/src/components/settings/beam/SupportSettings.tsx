...
import { Beam, SupportType } from '@/types/beam.ts';
...
function SupportConfig({ label, support, onUpdate, beamLength, colorClass }: SupportConfigProps) {
...
export function SupportSettings({ beam, onChange }: { beam: Beam; onChange: (beam: Beam) => void }) {
    const validation = useMemo(() => {
        if (beam.length === 0) return { error: null };
        if (beam.supportA.position === beam.supportB.position) {
            return { error: '支座A和B位置不能重叠' };
        }
        if (beam.supportA.position > beam.length || beam.supportB.position > beam.length) {
