...
import { Sidebar, SidebarContent, SidebarFooter, SidebarMenuSubItem } from "@/components/ui/sidebar.tsx";
...
export default function SectionSettingsBar({
    onChangeFormula, onChangeForceX, onChangeForceY, onChangeForceZ,
    onChangeForcePointY, onChangeForcePointZ, onCalculate
        ...
        <Sidebar side="right" variant="floating">
            <SidebarContent>
                ...
                <CollapsibleSidebarMenu icon={MousePointer2} title="外力作用点">
                    <SidebarMenuSubItem className="flex">
                        <Label>y:</Label>
                        <Input onChange={(e) => onChangeForcePointY(Number(e.target.value))} />
                        <Label>z:</Label>
                        <Input onChange={(e) => onChangeForcePointZ(Number(e.target.value))} />
                    </SidebarMenuSubItem>
                </CollapsibleSidebarMenu>
            </SidebarContent>
            <SidebarFooter>
                <Button className="w-full" onClick={onCalculate}>计算正应力</Button>
            </SidebarFooter>
        </Sidebar>
    );
}