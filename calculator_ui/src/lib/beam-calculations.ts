import { Beam, SolverError, SupportType, LoadType, Load } from '@/types/beam';
...
export async function sumForces(beam: Beam): Promise<Load[] | SolverError> {
    ...
    const hasFixedSupport = processedBeam.supportA.type === SupportType.Fixed || processedBeam.supportB.type === SupportType.Fixed;
    const hasTwoSupport = processedBeam.supportA.type !== SupportType.None && processedBeam.supportB.type !== SupportType.None;
    if (hasFixedSupport && hasTwoSupport) return {
...
// 计算约束反力
function calculateReactions(beam: Beam, hasFixedSupport: boolean): Load[] | null {
    // 计算所有外力的合力和合力矩
    let totalForce = 0;
    let totalMoment = 0;
    ...
    if (beam.supportA.type === SupportType.None || beam.supportB.type === SupportType.None)
        return null; // 静不定

    // 两个支座
    // 对支座A取矩：fb * (posB - posA) + totalMoment - totalForce * posA = 0
    const fb = -(totalMoment - totalForce * beam.supportA.position) / (beam.supportB.position - beam.supportA.position);
